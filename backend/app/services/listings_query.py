...
def _item(
    ...
    *,
    ...
    return {
        ...
        "title": title,
        "image_url": (raw_data or {}).get("image_urls", [None])[0]
        if (raw_data or {}).get("image_urls")
        else None,
        "price": price,
        "currency": currency,
        "mileage": mileage,
...
def list_listings(
    ...
    **filters,
    ...
    items = [
        ...
        for listing, vehicle, price, currency, mileage, title, raw_data, condition_signals in rows
    ]
...
def get_listing_detail(session: Session, listing_id: int) -> dict | None:
    ...
    from app.services.translation import language_for_country, translate_to_spanish

    def snapshot_data(snapshot):
        value = ListingSnapshotRead.model_validate(snapshot).model_dump()
        language = language_for_country(listing.country)
        value["description_es"] = translate_to_spanish(snapshot.description, language) if snapshot.description else None
        value["seller_comment_es"] = translate_to_spanish(snapshot.seller_comment, language) if snapshot.seller_comment else None
        return value

    data["current_snapshot"] = snapshot_data(latest) if latest else None
    data["snapshots"] = [
        snapshot_data(s)
    ...
    data["price_predictions"] = [
        PricePredictionRead.model_validate(p).model_dump()
        for p in session.scalars(
            select(PricePrediction).where(PricePrediction.listing_id == listing_id)
        ).all()
    ]
    from app.models import DealScore
    from app.schemas.deal_score import DealScoreRead
    deal_score = session.scalar(select(DealScore).where(DealScore.listing_id == listing_id))
    data["deal_score"] = DealScoreRead.model_validate(deal_score).model_dump() if deal_score else None
