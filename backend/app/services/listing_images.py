...
def _preferred_download_url(url: str) -> str:
    """Pide una variante grande a CDNs que exponen tamaños en la ruta."""
    if "prod.pictures.autoscout24.net" in url:
        import re
        return re.sub(r"/\d+x\d+\.(?:webp|jpg|jpeg)(?=$|\?)", "/1200x900.webp", url, flags=re.IGNORECASE)
    return url
...
def ensure_local_images(source: str, source_listing_id: str, urls: list[str]) -> dict:
    ...
    def _available(url: str) -> bool:
        ...
        preferred_url = _preferred_download_url(url)
        return bool(
            rec
            and rec.get("status") == "ok"
            and rec.get("download_url", url) == preferred_url
            and Path(rec.get("local_path") or "").exists()
        )
        ...
        with _image_client() as client:
            for index, url in enumerate(urls, start=1):
                if _available(url):
                    continue
                try:
                    response = client.get(_preferred_download_url(url))
                    ...
                    manifest[url] = {
                        "local_path": location,
                        "status": "ok",
                        "download_url": _preferred_download_url(url),
                    }
                    downloaded += 1
                ...
                "images": [
                    {
                        ...
                        "download_url": rec.get("download_url", url),
                        "error": rec.get("error"),
                    }
