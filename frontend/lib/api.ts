...
export interface DealScore {
  id: number; listing_id: number; sale_value: number | null; purchase_cost: number | null;
  import_cost: number; repair_cost: number; preparation_cost: number; financing_cost: number;
  expected_profit: number | null; roi: number | null; score: number | null;
  confidence: string; condition_bucket: string; model_version: string; calculated_at: string;
}
...
export async function updateListingStatus(
  ...
  return getJson<ListingDetail>(`/api/v1/listings/${id}/status`, {
    method: "PATCH",
