...
import { fetchFavorites, fetchListings, type ListingFilters, type ListingListItem } from "@/lib/api";
...
export function ClientListings({ filters }: { filters: ListingFilters }) {
  ...
  useEffect(() => {
    ...
    fetchFavorites().then((items) => setFavoriteIds(new Set(items.map((item) => item.listing_id)))).catch(() => {});
  }, [filters]);
  ...
  return <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{items.map((item) => <ListingCard key={item.id} listing={item} favorite={favoriteIds.has(item.id)} />)}</div>;
}
