...
import { conditionFindings, cosmeticFindings, hasCosmeticText } from "@/lib/condition";
...
export default async function ListingPage({
  ...
  const textProblems = (textSignals?.problem_types as string[] | undefined)
    ...
    ?? [];
  const findings = conditionFindings(listing.text_signals, listing.condition_signals, listing.photo_signals);
  const cosmeticDefects = cosmeticFindings(listing.photo_signals);
  const hasTextCosmetic = hasCosmeticText(listing.text_signals) || hasCosmeticText(listing.condition_signals);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      ...
      <StatusControl listingId={listing.id} currentStatus={listing.status} />

      {listing.price_predictions.length > 0 && (
        <Section title="Valoración de mercado">
          <div className="space-y-1 text-sm">
            {listing.price_predictions.map((prediction) => (
              <div key={prediction.id} className="flex flex-wrap justify-between gap-3 border-b border-neutral-100 py-2 last:border-0 dark:border-neutral-800">
                <span>{prediction.market === "es" ? "Mercado español" : "Mercado europeo"} · {prediction.condition_bucket} · confianza {prediction.confidence}</span>
                <span className="font-medium">P10 {prediction.p10?.toLocaleString("es-ES") ?? "—"} € · P50 {prediction.p50?.toLocaleString("es-ES") ?? "—"} € · P90 {prediction.p90?.toLocaleString("es-ES") ?? "—"} €</span>
              </div>
            ))}
          </div>
        </Section>
      ...
      {listing.deal_score && (
        <Section title="Deal Score">
          <div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
            <SignalRow label="Score" value={listing.deal_score.score != null ? listing.deal_score.score.toFixed(1) : "—"} />
            <SignalRow label="Beneficio neto" value={listing.deal_score.expected_profit != null ? `${listing.deal_score.expected_profit.toLocaleString("es-ES")} €` : "—"} />
            <SignalRow label="ROI" value={listing.deal_score.roi != null ? `${(listing.deal_score.roi * 100).toFixed(1)}%` : "—"} />
            <SignalRow label="Reparaciones" value={`${listing.deal_score.repair_cost.toLocaleString("es-ES")} €`} />
          </div>
        </Section>
      )}
      ...
      {findings.length > 0 && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-900 dark:border-red-900 dark:bg-red-950 dark:text-red-100">
          <strong>Averías detectadas:</strong>
          <ul className="mt-1 list-disc pl-5">
            {findings.map((finding) => <li key={`${finding.label}-${finding.source}`}>{finding.label} — fuente: {finding.source}</li>)}
          </ul>
        </div>
      )}
      {(cosmeticDefects.length > 0 || hasTextCosmetic) && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">
          <strong>Posible daño estético:</strong> {cosmeticDefects.join(", ") || "repintado/daño de carrocería"} — fuente: {cosmeticDefects.length > 0 ? "análisis visual CV" : "texto del anuncio"}.
        </div>
      )}
      ...
      {(listing.current_snapshot?.description_es || listing.current_snapshot?.seller_comment_es || listing.current_snapshot?.description || listing.current_snapshot?.seller_comment) && (
        <Section title="Información del vendedor">
          {listing.current_snapshot?.description_es && (
            <div className="mb-4">
              <h3 className="mb-1 text-sm font-semibold">Descripción traducida al español</h3>
              <p className="whitespace-pre-wrap text-sm text-neutral-700 dark:text-neutral-300">{listing.current_snapshot.description_es}</p>
              {listing.current_snapshot.description && listing.current_snapshot.description !== listing.current_snapshot.description_es && (
                <details className="mt-2 text-xs text-neutral-500"><summary className="cursor-pointer">Ver texto original</summary><p className="mt-1 whitespace-pre-wrap">{listing.current_snapshot.description}</p></details>
              )}
            </div>
          )}
          {listing.current_snapshot?.seller_comment_es && (
            <div>
              <h3 className="mb-1 text-sm font-semibold">Comentario del vendedor traducido al español</h3>
              <p className="whitespace-pre-wrap text-sm text-neutral-700 dark:text-neutral-300">{listing.current_snapshot.seller_comment_es}</p>
              {listing.current_snapshot.seller_comment && listing.current_snapshot.seller_comment !== listing.current_snapshot.seller_comment_es && (
                <details className="mt-2 text-xs text-neutral-500"><summary className="cursor-pointer">Ver comentario original</summary><p className="mt-1 whitespace-pre-wrap">{listing.current_snapshot.seller_comment}</p></details>
              )}
            </div>
          )}
          {!listing.current_snapshot?.description_es && listing.current_snapshot?.description && <p className="text-sm">{listing.current_snapshot.description}</p>}
          {!listing.current_snapshot?.seller_comment_es && listing.current_snapshot?.seller_comment && <p className="mt-3 text-sm">{listing.current_snapshot.seller_comment}</p>}
        </Section>
      )}
          ...
          <ul className="space-y-2">
            {listing.photo_analyses.map((a) => (
              <li key={a.id} className="flex items-center justify-between gap-4 text-sm">
                <a
                  href={preferredImageUrl(a.image_url)}
                  target="_blank"
                  rel="noreferrer"
                  className="truncate text-blue-600 underline decoration-blue-300 underline-offset-2 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"
